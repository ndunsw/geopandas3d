...
def plot3d(gdf3d, column: Optional[str] = None, cmap: str = "viridis", 
    ...
    if len(gdf3d) == 0:
        warnings.warn("Empty GeoDataFrame3D - nothing to plot")
        return None, None
    ...
    if not valid_mask.any():
        warnings.warn("No valid 3D coordinates found for plotting")
        return fig, ax
...
def plot_points_3d(points: np.ndarray, values: Optional[np.ndarray] = None,
    ...
    if len(points) == 0:
        warnings.warn("No points to plot")
        return None, None
...
def plot_polygons_3d(polygons: list, heights: list, figsize: Tuple[int, int] = (10, 8),
    ...
    if len(polygons) == 0:
        warnings.warn("No polygons to plot")
        return None, None
    ...
    for i, (polygon, height) in enumerate(zip(polygons, heights)):
        ...
        try:
            ...
            warnings.warn(f"Error plotting polygon {i}: {e}")
            continue
