...
import numpy as np
import pandas as pd
from typing import Iterable, Optional, Tuple, List, Union, Literal, Any
import warnings
...
from .utils import validate_3d_coordinates
...
class GeoDataFrame3D(GeoDataFrame):
    ...
    def from_polygons(cls, polygons: List[List[Tuple[float, float, float]]], 
        ...
        for polygon_vertices in polygons:
            if len(polygon_vertices) < 3:
                warnings.warn(f"Polygon with {len(polygon_vertices)} vertices skipped (need at least 3)")
                continue
            ...
            try:
                ...
                if poly.is_valid:
                    ...
                    warnings.warn(f"Invalid polygon skipped: {poly}")
            except Exception as e:
                warnings.warn(f"Error creating polygon: {e}")
                continue
    ...
    def sjoin_nearest3d(self, other: "GeoDataFrame3D", k: int = 1, how: Literal["left", "right", "inner"] = "left", 
        ...
        for i, (nbrs, dists) in enumerate(zip(idx, dist)):
            ...
            for j, (nbr_idx, d) in enumerate(zip(nbrs, dists)):
